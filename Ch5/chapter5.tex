...
%
%         
For the cases in \Cref{tab:1} the best fit to this scaling is with $c_1 \approx 1.8 \times 10^{-3}$  and  $c_2 \approx 2.0 \times 10^{-3}$. Comparison of the resulting scaling in (\ref{E:12}) and the actual computational times is shown in \Cref{F:5}. Note Cases 5a,b have been excluded since their large matrix sizes introduce additional time for memory management that are not representative of practical implementations. The fact that the scaling prefactors $c_1$  and  $c_2$ are nearly equal was unexpected in \Cref{sec:IIIF} and likely reflects the substantial speedup available in LAPACK for the symmetric matrix inversion compared to a brute force inversion.
It is apparent in \Cref{F:5} that (\ref{E:12}) provides reasonable scaling of $T$ over nearly three orders of magnitude, and can therefore be used to understand how the implementation parameters $N$ and $M$ affect the computational time. For this purpose it is useful to rearrange (\ref{E:12}) as
%
%    EQUATION   %  
%%%%%%%%%%%%%%%%%
\begin{equation}
...
\begin{figure}
	\begin{center}
	\includegraphics[width=\maxwidth{3.5in}]{Fig5}
...
As noted in \Cref{sec:IIB}, large bounding boxes allow for larger numbers $M$ of widely spaced (and thus more independent) training points, which provide more information for determining $F_{ij}$  via $\mathbf{h}_{ij}$. However, increasingly larger bounding boxes contain increasingly different local turbulence states than that which applies at the bounding box center point $\mathbf{x}$, and thus yield coefficients $\mathbf{h}_{ij}$  that are influenced by states other than that at $\mathbf{x}$. On the other hand, increasingly smaller bounding boxes provide a more-local implementation that assures training points are relevant to the local turbulence state at $\mathbf{x}$, but they inherently limit the number $M$ of available training points and their relative independence.  

To understand the performance of local (small $n^3$) and nonlocal (large $n^3$) implementations of autonomic closure, \Cref{F:6} shows probability densities of a typical subgrid stress component  $\tau_{ij}$ and the subgrid production $P$, and the scale-dependent support-density metrics $M_1$ and $M_2$  comparing the accuracy in the support-densities of the subgrid production fields. The red curves show the performance of local implementations (Cases 3a vs 4a) and the blue curves are from nonlocal implementations (Cases 6a vs 6b). Cases 3a and 6a have the same number $M/N$  of training points per degree of freedom, as do Cases 4a and 6b, and all these cases are second-order velocity-only implementations, so these comparisons are indicative of the effects of local versus nonlocal implementations. 

In \Cref{F:6} it is apparent in both $M_1$  and $M_2$  that the local implementations give more accurate results for the support on which large magnitudes of subgrid production are concentrated than do the nonlocal implementations. Support-density correlations $M_1$ for the local implementations are as high as $98\%$ and rms errors $M_2$  decrease to less than $2.3\%$  at the largest scale-ratio, while for the nonlocal implementations the correlations $M_1$  only reach $94\%$ and rms errors $M_2$  decrease only to $4\%$. The pdfs in  \Cref{F:6} show that, for the same $M/N$  value, the local implementations give better representation of  $\tau_{ij}$ and $P$, especially in the tails that correspond to large magnitudes, than do the corresponding nonlocal implementations. Moreover, note in \Cref{tab:1} that the computational cost is the same for Cases 3a and 6a, and for Cases 4a and 6b, since  $M/N$ is the same for each pair, as discussed in \Cref{sec:IVA}. We conclude that, all other factors being the same, local implementations are more accurate than nonlocal ones. 
...
\begin{figure}
	...
	\includegraphics[width=\maxwidth{6.5in}]{Fig6.pdf}
...
\begin{figure}
	...
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_normal_NG2.eps}}
	\subcaption{Typical normal subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i=j$}
	\label{F:NG2a}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_shear_NG2.eps}}
	\subcaption{Typical shear subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i \neq j$}
	\label{F:NG2b}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{P1x2_NG2.eps}}
	\subcaption{Corresponding subgrid production fields $P(\mathbf{x},t)$.}
	\label{F:NG2c}
\end{figure}
%
\begin{figure}
...
\begin{figure}
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_normal_CG24.eps}}
	\subcaption{Typical normal subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i=j$}
	\label{F:CG24a}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_shear_CG24.eps}}
	\subcaption{Typical shear subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i \neq j$}
	\label{F:CG24b}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{P1x2_CG24.eps}}
	\subcaption{Corresponding subgrid production fields $P(\mathbf{x},t)$.}
	\label{F:CG24c}
\end{figure}
%
...
\begin{figure}
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_normal_CL28.eps}}
	\subcaption{Typical normal subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i=j$}
	\label{F:CL28a}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_shear_CL28.eps}}
	\subcaption{Typical shear subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i \neq j$}
	\label{F:CL28b}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{P1x2_CL28.eps}}
	\subcaption{Corresponding subgrid production fields $P(\mathbf{x},t)$.}
	\label{F:CL28c}
\end{figure}
%
...
\begin{figure}
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_normal_NG2p.eps}}
	\subcaption{Typical normal subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i=j$}
	\label{F:NG2pa}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_shear_NG2p.eps}}
	\subcaption{Typical shear subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i \neq j$}
	\label{F:NG2pb}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{P1x2_NG2p.eps}}
	\subcaption{Corresponding subgrid production fields $P(\mathbf{x},t)$.}
	\label{F:NG2pc}
\end{figure}
%
...
\begin{figure}
	\centering \hspace{-1.0cm}
	\includegraphics[width=\maxwidth{6.5in}]{Fig7.pdf}
...
\begin{figure}
	\centering \hspace{-1.0cm}
	\includegraphics[width=\maxwidth{6.5in}]{Fig8.pdf}
...
\begin{figure}
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_normal_CL14.eps}}
	\subcaption{Typical normal subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i=j$}
	\label{F:CL14a}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_shear_CL14.eps}}
	\subcaption{Typical shear subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i \neq j$}
	\label{F:CL14b}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{P1x2_CL14.eps}}
	\subcaption{Corresponding subgrid production fields $P(\mathbf{x},t)$.}
	\label{F:CL14c}
\end{figure}
%
...
\begin{figure}
	\centering \hspace{-1.0cm}
	\includegraphics[width=\maxwidth{6.5in}]{Fig9.pdf}
...
A second-order implementation allows an even larger set of such parametric quantities, including tensor products of $S_{ij}$, $R_{ij}$,  $\nabla S_{ij}$ and $\nabla R_{ij}$, to be implicitly represented in $F_{ij}$. It is this large set of possible tensor products that makes an explicit tensor invariance-preserving parametric formulation of  $F_{ij}$ far more difficult to implement than is the primitive variable formulation in (\ref{E:5}) and (\ref{E:6}), for which tensor invariance properties are instead implicitly inherited in $F_{ij}$  from the test-stress training data, which intrinsically satisfies these properties. Results in \Cref{sec:IVF} and \Cref{ch:6} show that the present primitive-variable formulation produces far more accurate results for  $\tau_{ij}(\mathbf{x},t)$ and $P(\mathbf{x},t)$  than do traditional prescribed closure models that explicitly enforce the tensor invariance properties of the subgrid stress.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Effect of Number of Training Points and Their Separation}
\label{sec:IVE}

As noted in \Cref{sec:IIB}, both the number of training points and their relative independence might be expected to affect the amount of information available in the $\widehat{\mathbf{V}}$  matrix to determine the $N$ degrees of freedom in $F_{ij}$, and thereby increase its generalizability from the test scale to the LES scale. In general, large numbers $M$ of training points and increased separation $(V_B/M)^{1/3}$  between them could thus lead to improved accuracy in $\tau_{ij}(\mathbf{x},t)$  and the associated  $P(\mathbf{x},t)$. However, simply increasing both $M$ and $(V_B/M)^{1/3}$  simultaneously by making the bounding box volume  $V_B$ large was shown in \Cref{sec:IVB} to be ineffective, since the resulting less-local implementation then causes $\widehat{\mathbf{V}}$ to contain training points that are not relevant to the turbulence state at the bounding box center point $\mathbf{x}$. On the other hand, reducing  $V_B$ inherently limits the number of available training points and their relative separation. 
...
\begin{figure}
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_normal_CL18.eps}}
	\subcaption{Typical normal subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i=j$}
	\label{F:CL18a}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_shear_CL18.eps}}
	\subcaption{Typical shear subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i \neq j$}
	\label{F:CL18b}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{P1x2_CL18.eps}}
	\subcaption{Corresponding subgrid production fields $P(\mathbf{x},t)$.}
	\label{F:CL18c}
\end{figure}
%
...
\begin{figure}
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_normal_CL24.eps}}
	\subcaption{Typical normal subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i=j$}
	\label{F:10a}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{tau3x2_shear_CL24.eps}}
	\subcaption{Typical shear subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i \neq j$}
	\label{F:10b}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{P1x2_CL24.eps}}
	\subcaption{Corresponding subgrid production fields $P(\mathbf{x},t)$.}
	\label{F:10c}
\end{figure}
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Homogeneous Shear Turbulence (HST) \textit{A priori} Test Results}
\label{sec:IVHST}
...
\begin{figure}
	\begin{center} 
	\includegraphics[width=\maxwidth{4in}]{Lumley_HST_43_max.png}
...
% 	\includegraphics[width=\maxwidth{5in}]{hst_y43_DNS_Auto-03.eps}
...
\begin{figure}
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{HST_tau3x2_normal_CL24.eps}}
	\subcaption{Typical normal subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i=j$}
	\label{F:HST_CL24a}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{HST_tau3x2_shear_CL24.eps}}
	\subcaption{Typical shear subgrid stress fields $\tau_{ij}(\mathbf{x},t)$, $i \neq j$}
	\label{F:HST_CL24b}
\end{figure}
%
\begin{figure}
	\ContinuedFloat
	\centering %\hspace{1cm}
	\subfloat{\includegraphics[width=\maxwidth{6.5in}]{HST_P1x2_CL24.eps}}
	\subcaption{Corresponding subgrid production fields $P(\mathbf{x},t)$.}
	\label{F:HST_CL24c}
\end{figure}
%
...
%
 ...
 \Cref{F:HST_CL24c} compares the production fields $P^F(\mathbf{x},t)$ determined from Case 3a  with the corresponding results for the true fields $P(\mathbf{x},t)$. It is apparent that this implementation accurately represents local energy exchanges between the resolved and the subgrid-scale for high shear turbulence. Large positive and negative values in $P^F(\mathbf{x},t)$ are clustered in the regions at essentially the same location and of the same size and shape as in $P(\mathbf{x},t)$. This is similar to what is observed in \Cref{F:10}. 
...
\begin{figure}
	\begin{center}
	\includegraphics[width=\maxwidth{5in}]{P_DS_PDF.png}
...
\begin{figure}
	\begin{center}
	\includegraphics[width=\maxwidth{2.75in}]{M1_HIT_HST_P.png}
	\includegraphics[width=\maxwidth{2.75in}]{M2_HIT_HST_P.png}
...
\begin{figure}
	\begin{center}
	\includegraphics[width=\maxwidth{6in}]{Fig11.pdf}
...
% 
% CONCLUSION
% Test results for the autonomic closure on the HST dataset reveal that a non-parametric approach does not suffer from the local physical variations in   Since the autonomic closure is data-driven,  not physics-based, it doesn’t matter what the local physical state is. Therefore CL24 works equally well in HST dataset as it does in the HIT case. 
